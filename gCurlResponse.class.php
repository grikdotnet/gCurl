...
class gCurlResponse{
     ...
     *  throw new gCurlException(gCurlResponse::INTERRUPT_BODY_HANDLER) 
    ...
    public function headersHandler($ch, $header_line){
        ...
        if (!($this->flags & gCurl::FLAG_HTTP_OK)){
            ...
            if (!preg_match ($regexp, $header_line, $found)){
                ...
                throw new gCurlException(110,0,$header_line);
            }else{
                //ok, it's HTTP
                $this->flags |= gCurl::FLAG_HTTP_OK;
                if ($found[1] == 100) {
                    $this->flags |= gCurl::FLAG_CONTINUE;
                }
                //get response codes
        ...
        if ($header_len<3 && ( $header_line[0] == "\r" || $header_line[0] == "\n")){
            if ($this->flags & gCurl::FLAG_CONTINUE){
                $this->flags &=~ gCurl::FLAG_CONTINUE;
                $this->flags &=~ gCurl::FLAG_HTTP_OK;
                return $header_len;
            }
    ...
    function bodyHandlerIntermediate($ch,$data){
        try {
            $this->gCurlHandlers->bodyHandler($data);
        }catch (gCurlException $E){
            if ($E->getCode() == gCurlResponse::INTERRUPT_BODY_HANDLER ){
                return -1;
            }
            throw $E;
        }
